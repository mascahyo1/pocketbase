...
<head>
    ...
    <title>Post Categories - PocketBase (WebSocket)</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-50 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        <div class="mb-8">
            <h1 class="text-4xl font-bold text-gray-800 mb-2">Manajemen Post Categories (WebSocket)</h1>
            <p class="text-gray-600">Kelola kategori posting dengan PocketBase menggunakan WebSocket</p>
            <div class="mt-2">
                <span id="wsStatus" class="text-sm px-3 py-1 rounded-full bg-gray-200 text-gray-700">Disconnected</span>
            </div>
        </div>
    ...
    <script>
        ...
        function connectWebSocket() {
            ...
            updateWSStatus('Connecting...', 'bg-yellow-200 text-yellow-800');
            
            // PocketBase WebSocket endpoint untuk realtime
            ws = new WebSocket(`${WS_URL}/api/realtime`);
            
            ws.onopen = function() {
                console.log('WebSocket connected');
                updateWSStatus('Connected', 'bg-green-200 text-green-800');
                reconnectAttempts = 0;
                ...
                console.log('Subscribed to post_categories');
            };
            ...
            ws.onmessage = function(event) {
                try {
                    ...
                    // Check if it's a collection change event
                    if (data.action && data.record) {
                        console.log(`Category ${data.action}:`, data.record);
            ...
            ws.onerror = function(error) {
                ...
                updateWSStatus('Error', 'bg-red-200 text-red-800');
            };
            
            ws.onclose = function() {
                console.log('WebSocket disconnected');
                updateWSStatus('Disconnected', 'bg-gray-200 text-gray-700');
                ...
                if (reconnectAttempts < maxReconnectAttempts) {
                    ...
                    showAlert('WebSocket disconnected. Please refresh the page.', 'error');
                }
            };
        }
        ...
        function updateWSStatus(text, className) {
            ...
            statusEl.className = `text-sm px-3 py-1 rounded-full ${className}`;
        }
        ...
        async function loadCategories(page = 1) {
            try {
                ...
                const response = await fetch(`${BASE_URL}/api/collections/post_categories/records?page=${page}&perPage=${perPage}&sort=-created`);
        ...
        async function handleCreate(event) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    closeCreateModal();
                    event.target.reset();
                    // WebSocket akan auto-reload, tapi fallback jika tidak connected
